...
import {popupTransitions} from './popup.animations';
...
export interface PopupInitOptions {
  ...
  componentRef: ComponentRef<PopupContainerComponent>;
...
export interface OnPopupPositionChange {
  onContextualPopupPositionChange(positionInfo: PopupPositionInfo): void;
}

type PopupContent =
  | ComponentFactory<OnComponentFactoryInit<any> & OnPopupPositionChange>
  | TemplateRef<any>;

const $WINDOW = $(window);

@Component({
  selector: 'wb-popup-container',
  template: '<ng-container #placeholder></ng-container>',
  styleUrls: ['./popup-container.component.less'],
  animations: [popupTransitions],
})
export class PopupContainerComponent implements OnInit, AfterViewChecked {
  element: HTMLElement;

  @ViewChild('placeholder', {read: ViewContainerRef})
  private placeholder: ViewContainerRef;

  @HostBinding('@popupTransitions')
  private popupTransitionsState: string | undefined;

  private componentRef: ComponentRef<PopupContainerComponent>;
  private content: PopupContent;
  private contentComponentRef: ComponentRef<
    OnComponentFactoryInit<any> & OnPopupPositionChange
  >;
